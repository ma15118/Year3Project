...
## 1. Plot the HRV, removed mean and smoothed versions.
    ...
    #print(hrv_signal)
    hrv_cleaned = IQR_outlier(hrv_signal)
    hrv_noMean = remove_mean(hrv_signal)

    hrv_cleaned_noMean = remove_mean(hrv_cleaned)
    cleaned_time = np.linspace(start_time, stop_time, len(hrv_cleaned))
    uncleaned_time = np.linspace(start_time, stop_time, len(hrv_signal))
...
##        return signal
    ...
    hrv_cleaned_smoothed = sine_taper(hrv_cleaned_noMean)
    ...
    axs[0].plot(uncleaned_time, hrv_signal_smoothed,'Blue')
    #axs[0].plot(uncleaned_time,hrv_noMean,'Green')
    axs[0].plot(cleaned_time,hrv_cleaned_smoothed,'Black')
    ...
    axs[1].plot(cleaned_time,hrv_cleaned_smoothed,'Black')
    #axs[1].plot(cleaned_time,hrv_cleaned_noMean,'Red')
    axs[1].set_xlabel('Time [s]')
    axs[1].set_ylabel('')
    axs[1].set_title('HRV smoothed')
    ...
    my_list = np.linspace(start_time, start_time, len(hrv_signal))
